...
export const PEDAGOGY_KNOWLEDGE_VERSION = "iwc-pedagogy-2026-08-14";
...
Build an ADAPTIVE_ARTICLE_V1 tutorial with 3–6 dynamically named sections and 7–12 blocks. It must teach a decision, make a weak-versus-effective choice or hidden reasoning visible, provide a reusable tool or address a realistic pitfall, require guided active practice followed by a new-topic transfer, and end with a compact self-check. EXPLANATION, a CONTRAST or REASONING demonstration, TOOLKIT or PITFALLS, PRACTICE, and a final SUMMARY are outcome requirements; the exact mix and section names must serve the learner's diagnosed difficulty rather than a fixed course template. The selected block kinds must match the rendered blocks exactly. Target 25–35 minutes for argument, development, cohesion, or task-response skills; target 15–25 minutes for a focused grammar or lexical skill. Use new examples created for the lesson, never a locator or a long quotation from Version 1. PRACTICE has three or four active prompts across one or more blocks, includes SHORT_TEXT independent output, and includes an UNSEEN_TOPIC transfer task. Reference reasoning stays revealable and must not disclose the later timed paper's answers.
